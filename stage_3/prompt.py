...
# 注：v3（弱化 family_count + 放宽 reason 长度）在 beers 上使标准化类真错被大量否决
# （R 0.842->0.672）；v4（normal_samples/column_stats 抽到按列共享的"列参考信息"段 +
# evidence 截断）进一步恶化（R 0.589）——列级证据必须紧贴各可疑格注入，
# LLM 才稳定遵从"多种写法应判 FI"的原则。v5（追加 value_burst 判定原则）使 beers
# abv 百分号类标准化真错被"高频合法"话术大量否决（F1 0.975->0.912）：对 value_burst
# 强调"高频不等于合法"反而让 LLM 对无该证据的格反向推出"高频=合法"。三版均已回退，
# 维持 v2；value_burst 候选的召回由 verifier 的异常高频重复值保护（不依赖 prompt）兜底。
